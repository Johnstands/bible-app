...
- **Reading plans (done):** four plans (the Bible in a year, the New Testament in 90 days, the Gospels in a month, Psalms and Proverbs in a month) generated by `src/plans.ts` from chapter lengths (`list_chapters`), each day balanced by verses and re-aimed after a long chapter. Progress is stored as plan ids and day numbers in `user.db` (migration 3; `src-tauri/src/plans.rs`), with no dates to fall behind on: the next reading is the first day not done, days can be done in any order, and a streak counts consecutive days with anything finished. The **Plans** panel (P) lists what is under way and on offer; a strip under a chapter that belongs to the next day ticks it off in place.
- Audio is skipped. Phase 5 is otherwise done.
