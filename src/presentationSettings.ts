...
import { GRANULARITIES, PRESENT_THEMES_LIST, TRANSITIONS } from "./presentation";
import type { Granularity, PresentTheme, Transition } from "./presentation";
...
export const DEFAULT_PRESENTATION_PREFS: PresentationPrefs = { theme: "dark", granularity: "verse", transition: "fade" };
...
export function sanitizePresentationPrefs(raw: unknown): PresentationPrefs {
  ...
  return {
    ...
    transition: TRANSITIONS.includes(r.transition as Transition) ? (r.transition as Transition) : d.transition,
  };
}
